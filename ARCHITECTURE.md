...
- [Current and planned behavior](#1-what-exists-today-and-what-is-changing)
...
## 1. What exists today, and what is changing

The sandbox lifecycle, environment configuration, compatibility digest, SSH
generations, and machine attachment commands described here already exist.
...
environment-only, and makes `init` independent of sandbox state. Those commands
and changed behaviors must not be assumed available merely because they are
described below. The frontend implementation plans track the transition;
the closing verification updates this guide to describe only shipped behavior.

Today bare launch can also take environment configuration in preference to the
file, and `JAILBOX_EDITOR` can override the file's editor selection. Both
overrides are removed by the planned file-configured frontend. Current editor
discovery does not enforce the planned extension-inventory checks,
and editor settings duplicate terminal proxy variables already generated by
SSH. The intended ownership described below moves policy composition and editor
checks into the frontend while core retains remote session proxy policy.
...
```mermaid
flowchart TD
    Human[Developer and jailbox.conf] --> Frontend[Editor frontend — planned separation]
...
|---|---|---|
| Frontend | Parse files; select and check the editor; compose policy; obtain connection records; create isolated editor settings; launch the editor; create initial config | `host/frontend/` (pending migration) |
| Core | Validate environment policy; identify resources; build/select images; enforce lifecycle, mounts, network, SSH, and attachment rules | `host/core/` (pending migration) |
...
In the intended frontend, the human workflow is:
...
2. Parse the file's strict `KEY=value` grammar. It has fewer accepted value
   characters than environment configuration. The migration additionally
...
The planned init suggestions, in order, are `.env`, `.git/hooks`, `AGENTS.md`,
...
Read-only paths are ordered because mount precedence can matter. The planned
frontend work makes proxy reuse comparison agree with the digest's set semantics;
the existing network code still has an order-sensitive comparison.
...
### The planned frontend's remaining work
...
profile, and opens Remote SSH using the reported configuration, host, and path.
...
in the supported matrix. The planned migration removes the automatic image/editor
warning while retaining that documented restriction.
...
the planned frontend contributes anchors for the policy files it consumes.
