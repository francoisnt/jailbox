#!/bin/bash
# Real-engine adversarial lifecycle matrix. Runs once, after wrapper preparation,
...
for tool in podman ssh ssh-keygen git setsid; do
    command -v "$tool" >/dev/null || matrix_die "$tool is required"
done
[[ $(uname -s) = Linux ]] || matrix_die 'Linux is required'
podman image exists jailbox-test-debian || matrix_die 'run wrapper-images.sh first'
FIXTURE=$(mktemp -d /tmp/jailbox-e2e-lifecycle.XXXXXXXX)
FIXTURE=$(cd "$FIXTURE" && pwd -P)
PROJECT="$FIXTURE/project"
LOG="$ROOT/testlog/lifecycle-$(date +%Y%m%d-%H%M%S)-$$"
...
cleanup() {
    ...
    # A failed sweep retains ledger entries for a later exact-name retry.
    ledger_sweep_own_run
    if [[ ! -f "$LEDGER_FILE" ]]; then
        rm -rf "$FIXTURE"
    else
        printf 'Cleanup incomplete; fixture retained at %s\n' "$FIXTURE" >&2
        result=1
    fi
    exit "$result"
}
...
reset_fixture() {
    ...
    unset JAILBOX_CONFIG_MEMORY_LIMIT JAILBOX_CONFIG_EGRESS_ALLOW_0
}
network_disconnect() {
...
# Lifecycle commands and SSH probes have no interactive input in this suite.
lifecycle_each_row run_row </dev/null
