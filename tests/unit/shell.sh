...
for stream in stdin stdout; do
    terminal --expect refuse "--$stream-pipe" -- "$ROOT/src/jailbox" shell
    grep -q 'requires a terminal on both stdin and stdout' "$FIXTURE/shell.stderr"
done
for argument in -- --help command ''; do
    terminal --expect refuse -- "$ROOT/src/jailbox" shell "$argument"
    grep -q 'unexpected argument' "$FIXTURE/shell.stderr"
done
terminal --expect refuse -- "$ROOT/src/jailbox" --config missing shell
...
HOME="$FIXTURE/host-home" terminal -- "$ROOT/src/jailbox" shell
JAILBOX_CONFIG_DEV_IMAGE=private-image-value terminal --expect refuse -- "$ROOT/src/jailbox" shell
...
CONVERGENCE_SHELL_DIRECTORY="$FIXTURE/missing" terminal --expect refuse -- "$ROOT/src/jailbox" shell
grep -q 'No such file or directory' "$FIXTURE/shell.stderr"
assert_no_mutation
"$ROOT/src/jailbox" --help > "$FIXTURE/help"
