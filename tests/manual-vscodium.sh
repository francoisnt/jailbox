...
# opens VSCodium for each, prompts for failure notes, and logs everything.
#
...
declare -A RESULTS=()   # stage → pass | fail
...
EOF
    ...
    echo ""
    echo "  VSCodium is open. Run the checks listed at the start of this test run."
    echo ""

    # ── prompt ────────────────────────────────────────────────────────────────

    local result note
    read -rp "  Result for $stage — Enter=pass, type failure note=fail: " note || true
    if [[ -z "$note" ]]; then
        result=pass
    else
        result=fail
...
echo "╚══════════════════════════════════════════════════════════════════════╝"
echo ""
echo "Manual checks for each stage:"
echo ""
...
echo ""
echo "After each stage: press Enter for pass, or type a failure note before Enter."
echo ""
read -rp "Press Enter when ready to start launching stages..."

{
    ...
    echo "stages: ${stages[*]}"
    echo "manual checks:"
    echo "- VSCodium opens remotely over SSH"
    echo "- integrated terminal opens in the container"
    echo "blank result input means pass; nonblank input is a failure note"
    echo ""
} >> "$LOG_FILE"
