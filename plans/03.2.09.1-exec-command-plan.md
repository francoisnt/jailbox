...
Declare exec as a command accepting arguments and add its help in
`host/public-api.sh`.
...
Update the preflight command classification: keep `ssh-config`'s existing
Podman-free exemption until 03.2.11 deletes the command, since removing it
earlier would route a still-dispatched command through the full launch
preflight; `init` and the frontend launch paths keep their frontend-owned
